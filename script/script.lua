...
until game:IsLoaded() and LocalPlayer ~= nil
...
local Client = {
    ...
    logger = Logger.new(
        ...
        nil,
        nil,
        CONFIG.DEBUG and Logger.levels.DEBUG or Logger.levels.INFO
    ),
...
function Client:connect()
    ...
    self.connection.OnMessage:Connect(function(message)
        ...
        if packet.id == PACKET_ID.SERVER_EXECTUION_REQUEST then
            ...
            self.logger:debug(
                'Executing script with execution ID: ' .. executionId
            )
...
--// MAIN //
if getgenv().VSC_ROBLOX_EXECUTOR_EXISTS then
    Logger:warn("VSC Roblox Excutor (client) already exists!")
    return
end
getgenv().VSC_ROBLOX_EXECUTOR_EXISTS = true
